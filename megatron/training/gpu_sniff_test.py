...
def _gather_and_check(name, local_value):
    ...
    if rank == 0:
        ...
        min_deviation = mean * OUTLIER_MIN_DEVIATION_FRAC
        outlier_ranks = (
            [i for i, v in enumerate(vals) if abs(v - mean) > max(std, min_deviation)]
            if std > 0 else []
        )
        has_outliers = len(outlier_ranks) > 0

        print(f"  {name}: mean={mean:.2f}, min={min_val:.2f}, max={max_val:.2f}")
        if has_outliers:
            for i in outlier_ranks:
                print(f"    OUTLIER rank {i}: {vals[i]:.2f} ({(vals[i] - mean) / mean:+.1%})")
...
def run_sniff_tests(
    ...
    if rank == 0:
        print(f"\n{'=' * 60}")
        print(f"  GPU sniff test{f' — {tag}' if tag else ''}")
        print(f"{'=' * 60}")

    any_outliers = False
    for name, value in bench_gemms(extra_shapes=extra_gemm_shapes):
        if _gather_and_check(name, value):
            any_outliers = True
...
def main():
    ...
    if rank == 0:
        print(f"GPU sniff test (standalone): world_size={world_size}, ep_size={ep_size}, tp_size={tp_size}")
